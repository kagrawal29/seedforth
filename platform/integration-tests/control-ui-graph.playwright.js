async (page) => {
  ...
  await page.locator('#token').fill('synthetic-browser-credential-not-a-secret');
  await page.getByRole('button',{name:'Connect',exact:true}).click();
  const reconnectedPlatform = page.locator('.portfolio-row').filter({hasText:'SeedForth Platform'}).first();
  await reconnectedPlatform.waitFor({state:'visible'});
  await reconnectedPlatform.getByRole('button',{name:'Open project',exact:true}).click();
  await page.locator('#board .card').first().waitFor({state:'visible'});
  await page.getByRole('button',{name:/^Verify current baseline and writer census/}).waitFor({state:'visible'});
