...
func CmdVersion(d *CmdDeps) error {
	_, err := fmt.Fprintf(d.Stdout, "maverick %s\n", versionString())
	return err
}
...
const configInitTemplate = `# maverick config
[targets.dev]
...
func CmdShell(d *CmdDeps) error {
	...
	if writeVerbRE.MatchString(cypher) {
		return fmt.Errorf("shell refuses write verbs (CREATE/MERGE/DELETE/SET/REMOVE/DROP/DETACH); use maverick-dev for writes")
	}
	rows, err := d.Drv.Run(cypher)
	if err != nil {
...
func CmdAsk(d *CmdDeps) error {
	if strings.TrimSpace(d.EmbedEndpoint) == "" {
		return fmt.Errorf("embed endpoint not configured — set MAVERICK_EMBED_ENDPOINT or [maverick].embed_endpoint in config")
	}
	if d.Flags == nil || len(d.Flags.Args) == 0 {
...
func CmdExportGuide(d *CmdDeps) error {
	...
	md.WriteString("<!-- AUTOGENERATED from :ContributorGuide {name: 'maverick'}\n")
	md.WriteString(fmt.Sprintf("Generated at: %s\n", time.Now().UTC().Format(time.RFC3339)))
	md.WriteString("Do NOT edit this file manually. Edit sections in the graph; re-run `maverick export-guide`.\n")
	md.WriteString("-->\n\n")
