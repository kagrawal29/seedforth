// Package help emits deterministic help text for the mycelium CLI.
//
...
// to mycelium-dev on the user's machine — required only for contributors.
package help
...
func Text() string {
	...
	fmt.Fprintf(&b, "mycelium %s — the team's living knowledge graph\n\n", ver)
	b.WriteString("USAGE\n")
	b.WriteString("  mycelium [FLAGS] <command> [args...]\n\n")

	b.WriteString("FLAGS\n")
	b.WriteString("  --target <dev|staging|prod>  target environment (default: from MYCELIUM_TARGET or config)\n")
	...
	b.WriteString("WRITE / ORCHESTRATION COMMANDS (require mycelium-dev on PATH)\n")
	for _, r := range dispatch.Registry() {
	...
	b.WriteString("docs/contributor-setup.md in the mycelium repo.\n")
