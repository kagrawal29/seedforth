...
WHERE 'work.control' IN owner_grant.permissions
  ...
  AND $scope IN ['flowing-indian','cajon-sensei']
MERGE (p:Principal {node_id:$principal})
ON CREATE SET p.kind='human',p.created_at=datetime()
SET p.enabled=true
MERGE (g:Grant {node_id:'grant-'+$principal+'-'+$scope})
SET g.scope=$scope,g.revoked=false,g.permissions=['read','conversation.read','conversation.send'],
